...
class PHPExcel_Reader_HTML extends BaseReader
{
    /**
    ...
    private function processDomElementThTdOther(Worksheet $sheet, int &$row, string &$column, string &$cellContent, DOMElement $child, array &$attributeArray): void
    {
        if ($child->nodeName !== 'td' && $child->nodeName !== 'th') {
            ...
            // CUSTOM
            if (isset($this->formats[$child->parentNode->parentNode->nodeName])) {
                $sheet->getStyle($column . $row)->applyFromArray($this->formats[$child->parentNode->parentNode->nodeName]);
            }
