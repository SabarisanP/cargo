{if $schedulesexpire|count==0} 
    <div class="well well-large"> Sorry! No expired schedule(s) available.</div>
{else}  
    <div class="table-responsive">
        <table class="table info_table table-hover" id="expire-schedules">
            <thead>
                <tr class="noborder">
                    ...
                    <th>
                        <div class="pull-left"> 
                            <span class='txt_green' ><i class="fa fa-list"></i> {$schedulesexpire|count} &nbsp;Found</span>
                        </div>
                    </th>
                </tr>
            </thead>
            <tbody>
                {foreach from=$schedulesexpire item=schs key=keyname}
                    ...
                    <tr {if $smarty.foreach.inssch.index==0} class="row-noborder" {/if}>
                        <td>
                            <div class="pull-left">
                                <h5>{$row['name']}</h5>
                            ...
                            <div class="pull-left">
                                ...
                                {if $row['scheduleTagList']}
                                    <span id="sch-tag-{$row['code']}" class="sch-tag" data-popover='popover' data-toggle='hover' data-container='body'
                                          data-content ='{foreach from=$row['scheduleTagList'] item=batag} {$batag->name} <br/>{/foreach}'
                                          >
                        ...
                        <td>
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu sch">
                                    <li>
                                        <a href="#schedules/edit-schedules?r={$estr|md5}&t=0&i={$row['code']}&type=expired" title="Edit schedules">Edit schedules</a>
                                    </li>
                                    <li>
                                        {assign var=rstr value=10|mt_rand:2000}
                                        <a href="#schedules/edit-schedules?r={$rstr|md5}&t=1&i={$row['code']}&type=expired" title="Edit Station">
                                            Edit Station
                                        </a>
                                    </li>
                                    <li> {assign var=stid value=10|mt_rand:2000}                                        
                                        <a href="#schedules/edit-schedules?r={$stid|md5}&t=3&i={$row['code']}&type=expired" title="Edit routes">
                                            Edit Routes
                                        </a>
                                    </li>
                                </ul>
                            ...
                            <!--  <a href="javascript:;" title="Close schedules" class="btn btn-default btn-xs"><i class="fa fa-times"></i></a>
                            -->
