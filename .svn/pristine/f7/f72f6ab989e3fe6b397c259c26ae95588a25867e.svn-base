<script type="text/html" id="local-transit-lr-body">
    {assign var="total_lr" value=0}
    {assign var="total_art" value=0}
    {foreach from=$local_transit_load_lr item=row key=k}
        <tr id="{$k}-to-branch">
            <td>{$row['name']}</td>
            <td>{$row['article']} <span class="local-transit-loaded-art text-muted text-small"></span></td>
            <td>{$row['lr']} <span class="local-transit-loaded-lr text-muted text-small"></span></td>
        </tr>
    ...
    <tr class="active bold">
        <td>Total</td>
        <td>{$total_art} <span class="text-muted text-small" id="local-transit-total-loaded-art" style="color: #fff;"></span></td>
        <td>{$total_lr} <span class="text-muted text-small" id="local-transit-total-loaded-lr" style="color: #fff;"></span></td>
    </tr>
</script>
...
<div id="localAlphabetOrder">
    {foreach from=$alphabet_letters item=row}
        <button onclick="scrollingLocal('{$row['alphabet']}', 'LCLD');" data-letter="{$row['alphabet']}" class="box-button scroll-link">{$row['alphabet']}</button>
    {/foreach}
    ...
    <div class="pull-right d-flex"> 
        <div class="getLrs d-flex" style="margin-right:20px;">
            ...
            <button onclick="getCargoRow('local','{$body['queryCode']}','{$body['fromDate']}','{$body['toDate']}','{$body['fromStationCode']}','{$body['toStationCode']}','{$body['viaStationCodes']}','{$body['transitOrganizationCode']}','{$body['transitTypeCode']}')" class="btn btn-success get-lr-btn">Get LR</button>
        </div>
        <div>
    ...
    <div>
        <table id="new-lr-table" class="table table-bordered " style="width:99%">
            ...
            <tbody class="body_table" id="local-transit-ogpl-list">
                ...
                {foreach item=list from=$local_transit_load_lr}
                    <tr>
                        <td colspan="14" align="left" class="active LCLD_leter_tr_{$list['name']|substr:0:1}">
                            <div class="d-flex align-items-center">
                                <input type="checkbox" onchange="selectBranchLocalTransitLrs(this, '{$list['name']}');checkHandleLocal();" />
                                <span class="mr_l_1_em d-flex align-items-center">{$list['name']}</span>
                                <select class="form-control localBranchSelect1" data-list-name="{$list['name']}"  style="max-width: 150px; margin:5px;">
                                    {if $tload_branches|count>0}
                                        <option value="">Select a branch</option>
                                    {/if}
                                    {foreach item=branch from=$tload_branches}
                                        <option value="{$branch->code}" data-station="{$branch->station->code}">{$branch->name}</option>
                                    {/foreach}
                                </select>
                            </div>
                        </td>
                    ...
                    {foreach item=row from=$list['data'] key=t}
                        ...
                        <tr class="{if $allowloadstatus == 1}bg-success{elseif $allowloadstatus == 2}bg-warning{elseif $allowloadstatus == 3}bg-danger{/if}">
                            <td align="center">
                                {if $allowloadstatus == 1}
                                    <input type="checkbox" onclick="checkHandleLocal();" class="ogpl-new-localLoad-items" data-listname="{$list['name']}" data-tobranch="{$row->to_organization_code}" data-from="{$row->from_station_code}" data-toname="{$row->to_station_name}" data-to="{$row->to_station_code}" data-oldtransitorg="{$row->transit_organization_code}" data-oldtransitstn="{$row->transit_station_code}"  value="{$row->cargo_code}" />
                                {/if}
                            </td>
                            <td align="center">
                            ...
                            <td align="left">
                                ...
                                {if $allowloadstatus == 1}
                                    <select class="local-transit-branch-sel form-control local-transit-branch localBranchSelect2" data-list-name="{$list['name']}">
                                        {if $tload_branches|count>0}
                                            <option value="">Select a branch</option>
                                        {/if}
                                        ...
                                        {foreach item=branch from=$tload_branches}
                                            {if $branch->station->code == $login_station}
                        ...
                        {if $action_rights['LR-SPLTUP'] == 1 && $cargo_setting->cargoPartitionFlag == 1} {* permision *}
                            ...
                            {/if}
                                <tr class="splt-row row-local row-{$row->cargo_code} row-local-{$row->cargo_code}">
                                    <td colspan="14">
                                                        ...
                                                        <div class="d-flex wrapDist artical-group artical{$art_id}">
                                                            ...
                                                            {if $row->cargo_type=="PBO"}
                                                                ...
                                                                {foreach from=$each_items item=box key=key}
                                                                    <div class="custom-checkbox local-{$row->cargo_code}">
                                                                        <input type="checkbox" onclick="checkAllValidation({$row->total_item_count},'{$row->cargo_code}','local');" id="checkbox{$art_id}_{$box}-local" value="{$box}" data-value="{$art_id}" data-artname="{$art_name}" data-artqty="{$art_qty}" data-total="{$art_qty}" data-cargoid="{$row->cargo_code}" class="custom-control-input localload" data-tobranch="{$row->to_organization_code}" data-from="{$row->from_station_code}" data-to="{$row->to_station_code}" data-oldtransitorg="{$row->transit_organization_code}" data-oldtransitstn="{$row->transit_station_code}">
                                                                        <label class="custom-control-label" for="checkbox{$art_id}_{$box}-local"> {$box}</label>
                                                                    </div>
                                                                {/foreach}
                                                            {else}
