...
function removeArray(val){
    var index = dilg.indexOf(val);
    if (index > -1) {
        dilg.splice(index, 1);
    }
}

// to reduce the color opacity to some level
function hexToRGBA(hex, alpha) {
    hex = hex.replace("#", "");
    var r = parseInt(hex.substring(0, 2), 16) + 24;
    var g = parseInt(hex.substring(2, 4), 16) -7;
    var b = parseInt(hex.substring(4, 6), 16) -22;
    r = r <= 255 ? (r >= 0 ? r : 0) : 255;
    g = g <= 255 ? (g >= 0 ? g : 0) : 255;
    b = b <= 255 ? (b >= 0 ? b : 0) : 255;
