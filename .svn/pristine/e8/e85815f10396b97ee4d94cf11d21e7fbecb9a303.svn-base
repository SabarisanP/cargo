...
<div class="tab-content" id="ogpl-trip-list">
    ...
    <div id="intransit_pane" class="tab-pane fade">{call ogplList data=$tabs['DISPATCH']}</div>
...
{function name="ogplList" data=[]}
    {foreach from=$data item=row key=k}
        {* {if $row->cargoActivityType->code == 'TRNT' || $row->cargoActivityType->code == 'HBTRNT'}
            {continue}
        {/if} *}
        ...
        {assign var="orgin_startTrip" value=in_array('ORGN-START-TRIP', $row->activity)}
        {assign var=consolidateEWayBill value=array()}

        {* {assign var="is_empty_and_load " value=false}
        {if is_array($activity_types) && in_array('INTRNT', array_column($activity_types, 'code')) && in_array('OTFD', array_column($activity_types, 'code'))}
            {$is_empty_and_load = true}
            {$isEmptyLoad = false}
        {/if} *}
        ...
        <div class="panel panel-default transit-row p-0" id="transit-{$row->code}" data-transitcode="{$row->code}" data-aliascode="{$row->aliasCode}" data-ogplstatus="{$row->ogplStatus}" data-vehiclecode="{$row->busVehicle->code}" >
                    ...
                    <div class="col-md-3 b_r">
                        <div>
                            ...
                            {foreach from=$row->activityType item=type}
                                {* {if $type->code != 'HBTRNT' && $type->code != 'TRNT'} *}
                        ...
                        <h5>{$row->fromOrganization->name|default:$row->fromStation->name} <img src="assets/img/ogpl/r.png" width="24px"> {$row->toOrganization->name|default:$row->toStation->name}</h5>
                        <b class="h4 bold">{$row->aliasCode}</b>
                        {if $row->viaOrganizations|count > 0}
                            ...
                            {foreach from=$row->viaOrganizations item=via key=i name=v}
                                {$via->name}<br/>
                            {/foreach}
            ...
            <ul class="transit-tabs">
                ...
                {assign var="is_transit_branch" value=in_array($login_branch,array_column($row->transitOrganizations, 'code'))}
                {* arrival tap *}
                {if $tab == 'ARRIVAL'}
                    {if $outForDelivery}
                    ...
                    {/if}
                    {if !$isEmptyLoad}
                        <li class="transit-tab b_l"><a class="tab-link" href="#cargo/local-transit-chart-details?transitCode={$row->code}&date={$row->tripDate}" title="View Chart" target="_blank">View Chart <i class="fa fa-arrow-right"></i></a></li>
                    {/if}
                    {if $cargo_setting->ogplModelCode == 'VER3'}
                        {if $cargo_setting->transitOdometerModelCode == 'RTEBSD'} 
                            {if $row->fromOrganization->code != $login_branch}
                               <li class=""><a class="btn btn-info view-chart-btn" href="#cargo/local-transit-unload-lr?transitCode={$row->code}&date={$row->tripDate}&vehicleCode={$row->busVehicle->code}&unloadType={$unloadType}&redirect=unload&transitType={$transittype}" onclick="{if $isEmptyLoad}acceptEmptyUnload('{$row->code}', '{$row->busVehicle->code}', '{$row->toOrganization->code}'){else}acceptUnloading();{/if}" title="Unload Transit" target="_blank">Unload</a></li>
                            {/if}
                        {else}
                            {if $isEmptyLoad && !$orgin_startTrip }
                ...
                {else}
                    {* 3 departure/intransit tap  & 4.completed tap*}
                    {if $outForDelivery}
                        {if isset($row->transitCargoDetails)}
                            {foreach from=$row->transitCargoDetails item=cargoDetail}
                                {if isset($cargoDetail->transitDetailsType->code) && $cargoDetail->transitDetailsType->code == 'OFDLD'}
                                    <li class=""><a class="btn btn-success view-chart-btn " href="#cargo/out-for-delivery-unload-lr?transitCode={$row->code}&date={$row->tripDate}&vehicleCode={$row->busVehicle->code}&transitType={$transittypeout}" title="Unload" target="_blank">Deliver</a></li>
                                    {break}
                                {/if}
                            {/foreach}
                    ...
                    {if !$isEmptyLoad}
                        <li class=""><a class="btn btn-primary view-chart-btn" href="#cargo/local-transit-chart-details?transitCode={$row->code}&date={$row->tripDate}" title="View Chart" target="_blank">View Chart</a></li>
                    {/if}
                {/if}
            </ul>
        </div>
