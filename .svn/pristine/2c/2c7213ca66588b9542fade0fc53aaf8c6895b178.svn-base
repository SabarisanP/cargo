<div class="brand_top">
    <h3>Outstanding Payment Report</h3>
...
onclick= "{if ($namespaceCode == "seenutransports" || $namespaceCode == "seenutrans")} printDeliveryReport(); {else} window.print(); {/if}"><i
                                    ...
                                    <div class="col-md-12">
                                        <div class="">
                                            <div id="search-panel">
                                                ...
                                                <form id='con_bk_rpt' method="post" name="con_bk_rpt"
                                                    action="report/table-chart-list" onsubmit="return false">
                                                    <div class="row">

                                                        <div class="form-group col-md-4">
                                                            <label for="delivery_rpt_date_range" class="req">Date
                                                                Range</label>
                                                            <div class="input-group">
                                                                <input type="text" name="dateRange"
                                                                    class="form-control sdate"
                                                                    id="delivery_rpt_date_range" readonly="true"
                                                        ...
                                                        <div class="form-group col-md-2">
                                                            <label for="rev_rpt_date_to">Select Branch</label>
                                                            <div class="input-group col-md-12">
                                                                <select id="fromOrganizationCode"
                                                                    name="fromOrganizationCode" class="form-control">
                                                                    {foreach $branches as $bcode => $branch}
                                                                        <option value="{$bcode}" data-station="{$bcode}"
                                                                            data-stationName="{$branch}"
                                                                            {if $login_station == $bcode}selected{/if}>
                                                                            {$branch}
                                                                        </option>
                                                                    {/foreach}
                                                                </select>
                                                        ...
                                                        <div class="form-group col-md-2">
                                                        <label for="vendorCode">{lang('vendor')}</label>
                                                        <div class="input-group col-md-12">
                                                            <select id="vendorCode" name="vendorCode"  class="form-control">
                                                        ...
                                                        <div class="form-group col-md-2">
                                                            ...
                                                            <div class="input-group">
                                                                ...
                                                                <input type="hidden"  id="vendorCodes" value="">
                                                                <input type="hidden" name="code" id="code" value="">
                                                                <button class="btn btn-success" type="button"
                                                                    onclick="searchCargoTableChart();">Generate</button>&nbsp;
                                                                <button type="button" class="btn btn-default btn-clear"
                                                                    onclick="clearTableChart();">Clear</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </form>
...
{literal}
    <script>
        ...
        function exportCargoDeliveryExcel() {
            document.con_bk_rpt.action = "report/table-chart-list";
            document.con_bk_rpt.target = "ifrm_consolidate_downloader";
            $('#export').val(1);
            $('#print').val('');
            $('#fromCode').val($('#fromOrganizationCode').val());
            document.con_bk_rpt.submit();
            return;
        }

        function searchCargoTableChart() {
            ...
            data.organizationName = $('#fromOrganizationCode option:selected').attr("data-stationName");
            ...
            $.ajax({
                type: "POST",
                dataType: 'html',
                url: base_url + "report/table-chart-list",
                data: data,
