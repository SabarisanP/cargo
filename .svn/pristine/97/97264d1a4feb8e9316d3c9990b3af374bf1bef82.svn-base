...
{assign var="dmsdomain" value=config_item('dms_domain')}
{assign var="product" value=config_item('product')}
{assign var="zone" value=config_item('zone')}
{assign var="namespace" value=_get_namespace_id()}
{assign var="authtoken" value=_get_user_hash()}
...
<script>
    ...
    fileUpload.prototype.initUpload = function(options = {}) {
        ...
        this.startUpload = (param) => {
            ...
            uploader.setOption('url', '{$dmsdomain}/file/' + dms.token + '/' + param.imageCategoryCode + '/upload');
            if (uploader.files.length) {
                uploader.start();
    ...
    fileUpload.prototype.getDmsToken = function(dms) {
        $.ajax({
            type: "GET",
            url: '{$dmsdomain}/token/{$namespace}/{$product}/{$authtoken}/{$zone}/authorize',
            crossDomain: true,
            dataType: 'json',
            async: false,
            ...
            success: function(response) {
                if (response.status == 1) {
                    ...
                    $.each(response.data || [], function(i, row) {
                        $.ajax({
                            type: "HEAD",
                            url: '{$dmsdomain}/download/' + row.imageUrlSlug,
                            crossDomain: true,
                            ...
                            if (isImage) {
                                $tpl.find('.file').attr('src', '{$dmsdomain}/download/' + row.imageUrlSlug);
                            } else {
                                ...
                                $tpl.find('.filetype').html(ext).addClass(badge);
                                $tpl.on('click', function() {
                                    window.open('{$dmsdomain}/download/' + row.imageUrlSlug);
                                });
                            }
                    ...
                    if (!(response.data || []).length) {
                        gallerynode.append('<div class="well well-large" style="grid-column: 1 / -1;">No Results Found!</div>');
                    }
