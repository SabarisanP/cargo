...
{strip}    
    ...
    {for $iter=1 to $no_of_copy}
        ...
        <table width="100%" class="border-collapse font-10">
            ...
            <tr>
              <td width="85%" align="center" class="font-10">
                <img src="assets/img/clientlogo/challenge-xpress-logo.png" height="30">
                <div style="text-align: center !important; display: inline;">
                  {$summary->fromOrganization->address1|capitalize}, &nbsp;<br>
                  {$summary->fromOrganization->address2|capitalize}<br>
                  <span class="bold">{$summary->fromOrganization->contact}</span>&nbsp;
                  {if isset($summary->tax->gstin)}
                    | &nbsp;<span class="bold"> GST : {$summary->tax->gstin} </span>
                  {/if}
                </div>
              </td>
              <td width="15%">
