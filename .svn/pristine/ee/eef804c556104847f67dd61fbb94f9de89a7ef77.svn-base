<style>
    .company-name{ font-weight:bold; font-size: 15px; letter-spacing: 2px; text-shadow: 2px 2px 2px #3b3838; text-transform: uppercase;}
            ...
            <tbody>
                <tr>
                    <td colspan="2" align="center">                        
                        {if $summary->fromOrganization->code == 'ORL45DS278'} 
                            <img src="assets/img/clientlogo/jasminparcel-national.jpg" width="350"><br>
                            Head Office : {$nsprofile->namespaceProfile->address},&nbsp;{$nsprofile->namespaceProfile->city} - {$nsprofile->namespaceProfile->pincode} <br>
                        {else}
                            <img src="assets/img/clientlogo/jasminparcel.jpg" width="350"><br>
                            Head Office : No : 16, S.N Godwon, Near I Dream Theatre, Ware house, Royapuram - 600013 | 9788018882
                        {/if}   
                        <br>
                        Branch Office : {$summary->fromOrganization->address1|capitalize}, &nbsp;{$summary->fromOrganization->address2|capitalize} <br>
                        {$summary->fromOrganization->contact} &nbsp; | &nbsp; <span class="bold"> GSTIN : {if isNotNull($summary->tax->gstin)} {$summary->tax->gstin} {else} 33GQDPS0463J1ZO {/if} </span><br>
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="2">
                        <table class="bordered" cellpadding="3">
                            <tr>
                                <td>
                                    <span class="bold font-10">G.C.No :  {$summary->code}</span> <br>
                        ...
                        <table >
                            ...
                            <tr>
                                ...
                                <td class="bordered" cellpadding="3">
                                    Invoice No : {($invoiceDetail[1])? $invoiceDetail[1]: '' } <br>
                                    Invoice Date : {($invoiceDetail[2])? $invoiceDetail[2]: '' } <br>
                                    Invoice Value : {($invoiceDetail[0])? $invoiceDetail[0] : ''}
                                </td>
                                
                                <td colspan="2" class="bordered" align="center">                                                                        
                                    <div class="h-seperator" style="padding-bottom: 3px;">At Owner's Risk</div>                                    
                                    Undertaking charges payable by party                               
                                </td>
                            </tr>
                        </table> 
                    ...
                    <td colspan="2">  
                        <table cellpadding="3" class="bordered">
                            <tr>
                                ...
                                <td align="center">
                                    {if $summary->fromOrganization->code == 'ORL45DS278'} 
                                        For National Express Parcel Service
                                    {else}
                                        For {$namespace_name}
                                    {/if} 
                                    <br><br>
