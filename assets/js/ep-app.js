...
/**
 * @version 2.1.3
 * @overview Ezee Print Connector
 * <p/>
 * Connects a web client to the Ezee Print software.
...
///// PRIVATE METHODS /////
    ...
    var _ep = {
        ...
        log: {
            /** Debugging messages */
            trace: function() { if (_ep.DEBUG) { console.log.apply(console, arguments); } },
            /** General messages */
            info: function() { console.info.apply(console, arguments); },
            /** General warnings */
            warn: function() { console.warn.apply(console, arguments); },
            /** Debugging errors */
            allay: function() { if (_ep.DEBUG) { console.warn.apply(console, arguments); } },
            /** General errors */
        ...
        websocket: {
            ...
            /** Default parameters used on new connections. Override values using options parameter on {@link ep.websocket.connect}. */
            connectConfig: {
                host: ["localhost", "localhost.ezeeinfo.in"], //hosts Ezee Print can be running on
                ...
                port: {
                    secure: [8181, 8282, 8383, 8484],   //list of secure ports Ezee Print could be listening on
                    insecure: [8182, 8283, 8384, 8485], //list of insecure ports Ezee Print could be listening on
            ...
            setup: {
                ...
                findConnection: function(config, resolve, reject) {
                    ...
                    if (!config.port.secure.length) {
                        if (!config.port.insecure.length) {
                            ...
                            _ep.log.error("No secure ports specified - forcing insecure connection");
                            config.usingSecure = false;
                        }
                    } else if (!config.port.insecure.length && !config.usingSecure) {
                        _ep.log.trace("No insecure ports specified - forcing secure connection");
                    ...
                    var deeper = function() {
                        ...
                        if ((config.usingSecure && config.port.portIndex >= config.port.secure.length)
                            ...
                            if (config.hostIndex >= config.host.length - 1) {
                                ...
                                reject(new Error("Unable to establish connection with EP"));
                                return;
                            } else {
                                config.hostIndex++;
                        ...
                        _ep.websocket.setup.findConnection(config, resolve, reject);
                    };
                    ...
                    try {
                        _ep.log.trace("Attempting connection", address);
                        _ep.websocket.connection = new _ep.tools.ws(address);
                    }
                    catch(err) {
                        _ep.log.error(err);
                        deeper();
                    ...
                    if (_ep.websocket.connection != null) {
                        _ep.websocket.connection.established = false;

                        //called on successful connection to ep, begins setup of websocket calls and resolves connect promise after certificate is sent
                        _ep.websocket.connection.onopen = function(evt) {
                            if (!_ep.websocket.connection.established) {
                                _ep.log.trace(evt);
                                _ep.log.info("Established connection with Ezee Print on " + address);

                                _ep.websocket.setup.openConnection({ resolve: resolve, reject: reject });

                                if (config.keepAlive > 0) {
                                    var interval = setInterval(function() {
                                        if (!_ep.tools.isActive()) {
                                            clearInterval(interval);
                                            return;
                                        }

                                        _ep.websocket.connection.send("ping");
                        ...
                        _ep.websocket.connection.onclose = function() {
                            // Safari compatibility fix to raise error event
                            if (_ep.websocket.connection && typeof navigator !== 'undefined' && navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1) {
                                _ep.websocket.connection.onerror();
                            }
                        };

                        //called for errors during setup (such as invalid ports), reject connect promise only if all ports have been tried
                        _ep.websocket.connection.onerror = function(evt) {
                            _ep.log.trace(evt);

                            _ep.websocket.connection = null;
                ...
                openConnection: function(openPromise) {
                    _ep.websocket.connection.established = true;

                    //called when an open connection is closed
                    _ep.websocket.connection.onclose = function(evt) {
                        _ep.log.trace(evt);

                        _ep.websocket.connection = null;
                        _ep.websocket.callClose(evt);
                        _ep.log.info("Closed connection with Ezee Print");

                        for(var uid in _ep.websocket.pendingCalls) {
                            if (_ep.websocket.pendingCalls.hasOwnProperty(uid)) {
                                _ep.websocket.pendingCalls[uid].reject(new Error("Connection closed before response received"));
                            }
                        }
                    ...
                    _ep.websocket.connection.onerror = function(evt) {
                        _ep.websocket.callError(evt);
                    };

                    //send JSON objects to ep
                    _ep.websocket.connection.sendData = function(obj) {
                        _ep.log.trace("Preparing object for websocket", obj);
                        ...
                        if (obj.promise != undefined) {
                            obj.uid = _ep.websocket.setup.newUID();
                            _ep.websocket.pendingCalls[obj.uid] = obj.promise;
                        }

                        //ensure we know how this was signed
                        obj.signAlgorithm = _ep.security.signAlgorithm;
                        ...
                        try {
                            if (obj.call != undefined && obj.signature == undefined) {
                                ...
                                var hashing = _ep.tools.hash(_ep.tools.stringify(signObj));
                                if (!hashing.then) {
                                    hashing = _ep.tools.promise(function(resolve) {
                                        resolve(hashing);
                                    });
                                }

                                hashing.then(function(hashed) {
                                    return _ep.security.callSign(hashed);
                                }).then(function(signature) {
                                    _ep.log.trace("Signature for call", signature);
                                    obj.signature = signature;

                                    _ep.signContent = undefined;
                                    _ep.websocket.connection.send(_ep.tools.stringify(obj));
                                });
                            } else {
                                _ep.log.trace("Signature for call", obj.signature);
                            ...
                            if (obj.promise != undefined) {
                                ...
                                delete _ep.websocket.pendingCalls[obj.uid];
                            }
                        }
                    };

                    //receive message from ep
                    _ep.websocket.connection.onmessage = function(evt) {
                        var returned = JSON.parse(evt.data);

                        if (returned.uid == null) {
                            if (returned.type == null) {
                                //incorrect response format, likely connected to incompatible ep version
                                _ep.websocket.connection.close(4003, "Connected to incompatible Ezee Print version");

                            } else {
                                //streams (callbacks only, no promises)
                                switch(returned.type) {
                                    case _ep.streams.serial:
                                        if (!returned.event) {
                                            returned.event = JSON.stringify({ portName: returned.key, output: returned.data });
                                        }

                                        _ep.serial.callSerial(JSON.parse(returned.event));
                                        break;
                                    case _ep.streams.socket:
                                        _ep.socket.callSocket(JSON.parse(returned.event));
                                        break;
                                    case _ep.streams.usb:
                                        if (!returned.event) {
                                            returned.event = JSON.stringify({ vendorId: returned.key[0], productId: returned.key[1], output: returned.data });
                                        }

                                        _ep.usb.callUsb(JSON.parse(returned.event));
                                        break;
                                    case _ep.streams.hid:
                                        _ep.hid.callHid(JSON.parse(returned.event));
                                        break;
                                    case _ep.streams.printer:
                                        _ep.printers.callPrinter(JSON.parse(returned.event));
                                        break;
                                    case _ep.streams.file:
                                        _ep.file.callFile(JSON.parse(returned.event));
                                        break;
                                    default:
                                        _ep.log.allay("Cannot determine stream type for callback", returned);
                                        break;
                                }
                            }
                        ...
                        _ep.log.trace("Received response from websocket", returned);

                        var promise = _ep.websocket.pendingCalls[returned.uid];
                        if (promise == undefined) {
                            _ep.log.allay('No promise found for returned response');
                        } else {
                        ...
                        delete _ep.websocket.pendingCalls[returned.uid];
                    };
                    ...
                    function sendCert(cert) {
                        ...
                        ep.api.getVersion().then(function(version) {
                            _ep.websocket.connection.version = version;
                            _ep.websocket.connection.semver = version.toLowerCase().replace(/-rc\./g, "-rc").split(/[\\+\\.-]/g);
                            for(var i = 0; i < _ep.websocket.connection.semver.length; i++) {
                                try {
                                    if (i == 3 && _ep.websocket.connection.semver[i].toLowerCase().indexOf("rc") == 0) {
                                        // Handle "rc1" pre-release by negating build info
                                        _ep.websocket.connection.semver[i] = -(_ep.websocket.connection.semver[i].replace(/\D/g, ""));
                                        continue;
                                    }
                                    _ep.websocket.connection.semver[i] = parseInt(_ep.websocket.connection.semver[i]);
                                }
                                catch(ignore) {}

                                if (_ep.websocket.connection.semver.length < 4) {
                                    _ep.websocket.connection.semver[3] = 0;
                                }
                            }

                            //algorithm can be declared before a connection, check for incompatibilities now that we have one
                            _ep.compatible.algorithm(true);
                        }).then(function() {
                            _ep.websocket.connection.sendData({ certificate: cert, promise: openPromise });
                        });
                    }

                    _ep.security.callCert().then(sendCert).catch(sendCert);
                },
            ...
            dataPromise: function(callName, params, signature, signingTimestamp) {
                return _ep.tools.promise(function(resolve, reject) {
                    var msg = {
                        call: callName,
                    ...
                    _ep.websocket.connection.sendData(msg);
                });
            },
            ...
            callError: function(evt) {
                if (Array.isArray(_ep.websocket.errorCallbacks)) {
                    for(var i = 0; i < _ep.websocket.errorCallbacks.length; i++) {
                        _ep.websocket.errorCallbacks[i](evt);
                    }
                } else {
                    _ep.websocket.errorCallbacks(evt);
                }
            },
            ...
            callClose: function(evt) {
                if (Array.isArray(_ep.websocket.closedCallbacks)) {
                    for(var i = 0; i < _ep.websocket.closedCallbacks.length; i++) {
                        _ep.websocket.closedCallbacks[i](evt);
                    }
                } else {
                    _ep.websocket.closedCallbacks(evt);
                }
            }
        },
        ...
        printing: {
            /** Default options used for new printer configs. Can be overridden using {@link ep.configs.setDefaults}. */
            defaultConfig: {
                //value purposes are explained in the ep.configs.setDefaults docs
        ...
        serial: {
            ...
            callSerial: function(streamEvent) {
                if (Array.isArray(_ep.serial.serialCallbacks)) {
                    for(var i = 0; i < _ep.serial.serialCallbacks.length; i++) {
                        _ep.serial.serialCallbacks[i](streamEvent);
                    }
                } else {
                    _ep.serial.serialCallbacks(streamEvent);
                }
            }
        },
        ...
        socket: {
            ...
            callSocket: function(socketEvent) {
                if (Array.isArray(_ep.socket.socketCallbacks)) {
                    for(var i = 0; i < _ep.socket.socketCallbacks.length; i++) {
                        _ep.socket.socketCallbacks[i](socketEvent);
                    }
                } else {
                    _ep.socket.socketCallbacks(socketEvent);
                }
            }
        },
        ...
        usb: {
            ...
            callUsb: function(streamEvent) {
                if (Array.isArray(_ep.usb.usbCallbacks)) {
                    for(var i = 0; i < _ep.usb.usbCallbacks.length; i++) {
                        _ep.usb.usbCallbacks[i](streamEvent);
                    }
                } else {
                    _ep.usb.usbCallbacks(streamEvent);
                }
            }
        },
        ...
        hid: {
            ...
            callHid: function(streamEvent) {
                if (Array.isArray(_ep.hid.hidCallbacks)) {
                    for(var i = 0; i < _ep.hid.hidCallbacks.length; i++) {
                        _ep.hid.hidCallbacks[i](streamEvent);
                    }
                } else {
                    _ep.hid.hidCallbacks(streamEvent);
                }
            }
        },
        ...
        printers: {
            ...
            callPrinter: function(streamEvent) {
                if (Array.isArray(_ep.printers.printerCallbacks)) {
                    for(var i = 0; i < _ep.printers.printerCallbacks.length; i++) {
                        _ep.printers.printerCallbacks[i](streamEvent);
                    }
                } else {
                    _ep.printers.printerCallbacks(streamEvent);
                }
            }
        },
        ...
        file: {
            ...
            callFile: function(streamEvent) {
                if (Array.isArray(_ep.file.fileCallbacks)) {
                    for(var i = 0; i < _ep.file.fileCallbacks.length; i++) {
                        _ep.file.fileCallbacks[i](streamEvent);
                    }
                } else {
                    _ep.file.fileCallbacks(streamEvent);
                }
            }
        },
        ...
        security: {
            ...
            /** Called to create new promise (using {@link _ep.security.certHandler}) for certificate retrieval. */
            callCert: function() {
                if (typeof _ep.security.certHandler.then === 'function' || _ep.security.certHandler.constructor.name === "AsyncFunction") {
                    //already a promise
                    return _ep.security.certHandler();
                } else {
                    //turn into a promise
                    return _ep.tools.promise(_ep.security.certHandler);
                }
            },
            ...
            /** Called to create new promise (using {@link _ep.security.signatureFactory}) for signed calls. */
            callSign: function(toSign) {
                if (typeof _ep.security.signatureFactory.then === 'function' || _ep.security.signatureFactory.constructor.name === "AsyncFunction") {
                    //already a promise
                    return _ep.security.signatureFactory(toSign);
                } else {
                    //turn into a promise
                    return _ep.tools.promise(_ep.security.signatureFactory(toSign));
                }
            },
        ...
        tools: {
            ...
            promise: function(resolver) {
                ...
                if (typeof RSVP !== 'undefined') {
                    ...
                    _ep.log.error("Promise/A+ support is required.  See ep.api.setPromiseType(...)");
                }
            },
                ...
                for(var i = 0; i < data.length; i++) {
                    if (data[i].constructor === Object) {
                        ...
                        if (absolute) {
                            ...
                            data[i].data = _ep.tools.absolute(data[i].data);
                        }
                        if (data[i].options && typeof data[i].options.overlay === 'string') {
                            data[i].options.overlay = _ep.tools.absolute(data[i].options.overlay);
                        }
                    }
                }
            },
                    ...
                    for(var key in source) {
                        if (source.hasOwnProperty(key)) {
                            ...
                            if (source[key] && source[key].constructor && source[key].constructor === Object) {
                                ...
                                target[key] = _ep.tools.extend(clone, source[key]);
            ...
            versionCompare: function(major, minor, patch, build) {
                if (_ep.tools.assertActive()) {
                    var semver = _ep.websocket.connection.semver;
                    if (semver[0] != major) {
            ...
            isVersion: function(major, minor, patch, build) {
                return _ep.tools.versionCompare(major, minor, patch, build) == 0;
            },

            isActive: function() {
                return _ep.websocket.connection != null && _ep.websocket.connection.established;
            },
            ...
            assertActive: function() {
                ...
                throw new Error("A connection to EP has not been established yet");
            }
        },
        ...
        compatible: {
            ...
            data: function(printData) {
                if (_ep.tools.isVersion(2, 0)) {
                     ...
                     */
                    _ep.log.trace("Converting print data to v2.0 for " + _ep.websocket.connection.version);
            ...
            config: function(config, dirty) {
                ...
                if(_ep.tools.versionCompare(2, 1, 2, 11) < 0) {
                    if(config.spool) {
            ...
            networking: function(hostname, port, signature, signingTimestamp, mappingCallback) {
                ...
                if (_ep.tools.isVersion(2, 0)) {
                    return _ep.tools.promise(function(resolve, reject) {
                        _ep.websocket.dataPromise('websocket.getNetworkInfo', {
                            hostname: hostname,
                            port: port
                ...
                // Wrap 2.1
                return _ep.tools.promise(function(resolve, reject) {
                    _ep.websocket.dataPromise('networking.device', {
                        hostname: hostname,
                        port: port
                    }, signature, signingTimestamp).then(function(data) {
            ...
            /** Check if EP version supports chosen algorithm */
            algorithm: function(quiet) {
                //if not connected yet we will assume compatibility exists for the time being
                if (_ep.tools.isActive()) {
                    if (_ep.tools.isVersion(2, 0)) {
                        if (!quiet) {
                            _ep.log.warn("Connected to an older version of EP, alternate signature algorithms are not supported");
                        }
                        return false;
                    }
                }
        ...
        SHA: {
            ...
            hash: function(msg) {
                ...
                for(var i = 0; i < N; i++) {
                    ...
                    for(var t = 16; t < 64; t++) { W[t] = (_ep.SHA._dev1(W[t-2]) + W[t-7] + _ep.SHA._dev0(W[t-15]) + W[t-16]) & 0xffffffff; }
                    // 2 - initialise working variables a, b, c, d, e, f, g, h with previous hash value
                    a = H[0]; b = H[1]; c = H[2]; d = H[3]; e = H[4]; f = H[5]; g = H[6]; h = H[7];
                    // 3 - main loop (note 'addition modulo 2^32')
                    for(var t = 0; t < 64; t++) {
                        var T1 = h + _ep.SHA._sig1(e) + _ep.SHA._ch(e, f, g) + K[t] + W[t];
                        var T2 = _ep.SHA._sig0(a) + _ep.SHA._maj(a, b, c);
                ...
                return _ep.SHA._hexStr(H[0]) + _ep.SHA._hexStr(H[1]) + _ep.SHA._hexStr(H[2]) + _ep.SHA._hexStr(H[3]) +
                    _ep.SHA._hexStr(H[4]) + _ep.SHA._hexStr(H[5]) + _ep.SHA._hexStr(H[6]) + _ep.SHA._hexStr(H[7]);
            },

            // Rotates right (circular right shift) value x by n positions
            _rotr: function(n, x) { return (x >>> n) | (x << (32 - n)); },
            // logical functions
            _sig0: function(x) { return _ep.SHA._rotr(2, x) ^ _ep.SHA._rotr(13, x) ^ _ep.SHA._rotr(22, x); },
            _sig1: function(x) { return _ep.SHA._rotr(6, x) ^ _ep.SHA._rotr(11, x) ^ _ep.SHA._rotr(25, x); },
            _dev0: function(x) { return _ep.SHA._rotr(7, x) ^ _ep.SHA._rotr(18, x) ^ (x >>> 3); },
            _dev1: function(x) { return _ep.SHA._rotr(17, x) ^ _ep.SHA._rotr(19, x) ^ (x >>> 10); },
...
///// CONFIG CLASS ////
    ...
    function Config(printer, opts) {
        ...
        this.config = _ep.tools.extend({}, _ep.printing.defaultConfig); //create a copy of the default options
        ...
        this.setPrinter = function(newPrinter) {
            ...
            if(newPrinter && newPrinter.file) {
                // TODO: Warn for UNC paths too 
                if(newPrinter.file.indexOf("\\\\") != 0) {
                    _ep.log.warn("Printing to file is deprecated.  See");
                }
            }
         ...
         * @param newOpts {Object} The options to change. See <code>ep.configs.setDefaults</code> docs for available values.
         *
         * @see ep.configs.setDefaults
         */
        this.reconfigure = function(newOpts) {
            ...
            _ep.tools.extend(this.config, newOpts);
        };
        ...
        this.getOptions = function() {
            return _ep.compatible.config(this.config, this._dirtyOpts);
        };
    ...
    /**
     * Shortcut method for calling <code>ep.print</code> with a particular config.
     * @param {Array<Object|string>} data Array of data being sent to the printer. See <code>ep.print</code> docs for available values.
     ...
     *
     * @example
     * ep.print(myConfig, ...); // OR
     * myConfig.print(...);
     *
     * @see ep.print
     */
    Config.prototype.print = function(data, signature, signingTimestamp) {
        ep.print(this, data, signature, signingTimestamp);
    };
...
///// PUBLIC METHODS /////
    ...
    var ep = {
         ...
         * @namespace ep.websocket
         */
        websocket: {
            /**
             * Check connection status. Active connection is necessary for other calls to run.
             *
             * @returns {boolean} If there is an active connection with Ezee Print.
             *
             * @see connect
             *
             * @memberof  ep.websocket
             */
            isActive: function() {
                return _ep.tools.isActive();
            },

            /**
             * Call to setup connection with Ezee Print on user's system.
             *
             * @param {Object} [options] Configuration options for the web socket connection.
             *  @param {string|Array<string>} [options.host=['localhost', 'localhost.ezeeinfo.in']] Host running the Ezee Print software.
            ...
            connect: function(options) {
                return _ep.tools.promise(function(resolve, reject) {
                    if (_ep.tools.isActive()) {
                        reject(new Error("An open connection with Ezee Print already exists"));
                        return;
                    } else if (_ep.websocket.connection != null) {
                    ...
                    if (!_ep.tools.ws) {
                        ...
                        return;
                    } else if (!_ep.tools.ws.CLOSED || _ep.tools.ws.CLOSED == 2) {
                    ...
                    if (typeof location === 'undefined' || location.protocol !== 'https:') {
                        ...
                        if (typeof options.usingSecure === 'undefined') {
                            _ep.log.trace("Disabling secure ports due to insecure page");
                    ...
                    var attempt = function(count) {
                        ...
                        var delayed = function() {
                            var config = _ep.tools.extend({}, _ep.websocket.connectConfig, options);
                            _ep.websocket.setup.findConnection(config, resolve, nextAttempt)
                        };
                        if (count == 0) {
            ...
            /**
             * Stop any active connection with Ezee Print.
             *
             * @returns {Promise<null|Error>}
             *
             * @memberof ep.websocket
            ...
            disconnect: function() {
                return _ep.tools.promise(function(resolve, reject) {
                    if (_ep.tools.isActive()) {
                        _ep.websocket.connection.close();
                        _ep.websocket.connection.promise = { resolve: resolve, reject: reject };
                    } else {
                        reject(new Error("No open connection with Ezee Print"))
                    }
                });
            },
            ...
            setErrorCallbacks: function(calls) {
                _ep.websocket.errorCallbacks = calls;
            },
            ...
            setClosedCallbacks: function(calls) {
                _ep.websocket.closedCallbacks = calls;
            },

            /**
             * @deprecated Since 2.1.0.  Please use ep.networking.device() instead
             *
             ...
             * @memberof ep.websocket
             */
            getNetworkInfo: _ep.compatible.networking,
            ...
            getConnectionInfo: function() {
                if (_ep.tools.assertActive()) {
                    var url = _ep.websocket.connection.url.split(/[:\/]+/g);
        ...
        printers: {
             ...
             *
             * @memberof ep.printers
             */
            getDefault: function(signature, signingTimestamp) {
                return _ep.websocket.dataPromise('printers.getDefault', null, signature, signingTimestamp);
            },
            ...
            find: function(query, signature, signingTimestamp) {
                return _ep.websocket.dataPromise('printers.find', { query: query }, signature, signingTimestamp);
            },

            /**
             * Provides a list, with additional information, for each printer available to EP.
             *
             ...
             */
            details: function() {
                return _ep.websocket.dataPromise('printers.detail');
            },
             ...
             * @since 2.1.0
             *
             * @see ep.printers.setPrinterCallbacks
             *
            ...
            startListening: function(printers) {
                ...
                return _ep.websocket.dataPromise('printers.startListening', params);
            },
            ...
            stopListening: function() {
                return _ep.websocket.dataPromise('printers.stopListening');
            },
             ...
             * @since 2.1.0
             *
             * @see ep.printers.startListening
             *
             * @memberof ep.printers
             */
            getStatus: function() {
                return _ep.websocket.dataPromise('printers.getStatus');
            },
            ...
            setPrinterCallbacks: function(calls) {
                _ep.printers.printerCallbacks = calls;
            }
        },

        /**
         * Calls related to setting up new printer configurations.
         * @namespace ep.configs
         */
        configs: {
            /**
            ...
            setDefaults: function(options) {
                _ep.tools.extend(_ep.printing.defaultConfig, options);
            },
        ...
        print: function(configs, data) {
            ...
            for(var d = 0; d < data.length; d++) {
                _ep.tools.relative(data[d]);
                _ep.compatible.data(data[d]);
            }
            ...
            var sendToPrint = function(mapping) {
                ...
                return _ep.websocket.dataPromise('print', params, mapping.signature, mapping.timestamp);
            };
            ...
            chain.reduce(function(sequence, link) {
                ...
                return last;
            }, _ep.tools.promise(function(r) { r(); })); //an immediately resolved promise to start off the chain
        ...
        serial: {
             ...
             */
            findPorts: function() {
                return _ep.websocket.dataPromise('serial.findPorts');
            },
            ...
            setSerialCallbacks: function(calls) {
                _ep.serial.serialCallbacks = calls;
            },
            ...
            openPort: function(port, options) {
                ...
                return _ep.websocket.dataPromise('serial.openPort', params);
            },
             ...
             * @param {Object} options Serial port configuration updates. See <code>ep.serial.openPort</code> `options` docs for available values.
             *     For best performance, it is recommended to only set these values on the port open call.
             *
             * @returns {Promise<null|Error>}
             *
             * @see ep.serial.setSerialCallbacks
             *
             * @memberof ep.serial
             */
            sendData: function(port, data, options) {
                if (_ep.tools.versionCompare(2, 1, 0, 12) >= 0) {
                    ...
                    if (data.type && data.type.toUpperCase() == "FILE") {
                        data.data = _ep.tools.absolute(data.data);
                    }
                }
                ...
                return _ep.websocket.dataPromise('serial.sendData', params);
            },
            ...
            closePort: function(port) {
                return _ep.websocket.dataPromise('serial.closePort', { port: port });
            }
        },
        ...
        socket: {
            ...
            open: function(host, port, options) {
                ...
                return _ep.websocket.dataPromise("socket.open", params);
            },
            ...
            close: function(host, port) {
                ...
                return _ep.websocket.dataPromise("socket.close", params);
            },
            ...
            sendData: function(host, port, data) {
                ...
                return _ep.websocket.dataPromise("socket.sendData", params);
            },
            ...
            setSocketCallbacks: function(calls) {
                _ep.socket.socketCallbacks = calls;
            }
        },
        ...
        usb: {
            ...
            listDevices: function(includeHubs) {
                return _ep.websocket.dataPromise('usb.listDevices', { includeHubs: includeHubs });
            },
            ...
            listInterfaces: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.listInterfaces', deviceInfo);
            },
            ...
            listEndpoints: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.listEndpoints', deviceInfo);
            },
            ...
            claimDevice: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.claimDevice', deviceInfo);
            },
            ...
            isClaimed: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.isClaimed', deviceInfo);
            },
            ...
            sendData: function(deviceInfo) {
                ...
                if (_ep.tools.versionCompare(2, 1, 0, 12) >= 0) {
                    ...
                    if (deviceInfo.data.type && deviceInfo.data.type.toUpperCase() == "FILE") {
                        deviceInfo.data.data = _ep.tools.absolute(deviceInfo.data.data);
                    }
                }

                return _ep.websocket.dataPromise('usb.sendData', deviceInfo);
            },
            ...
            readData: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.readData', deviceInfo);
            },
            ...
            openStream: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.openStream', deviceInfo);
            },
            ...
            closeStream: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.closeStream', deviceInfo);
            },
            ...
            releaseDevice: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('usb.releaseDevice', deviceInfo);
            }
        },
         ...
         * Many of these calls can be accomplished from the <code>ep.usb</code> namespace,
        ...
        hid: {
            ...
            listDevices: function() {
                return _ep.websocket.dataPromise('hid.listDevices');
            },
             ...
             * @since 2.0.1
             *
             * @see ep.hid.setHidCallbacks
             *
             * @memberof ep.hid
             */
            startListening: function() {
                return _ep.websocket.dataPromise('hid.startListening');
            },
             ...
             */
            stopListening: function() {
                return _ep.websocket.dataPromise('hid.stopListening');
            },
            ...
            claimDevice: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('hid.claimDevice', deviceInfo);
            },
            ...
            isClaimed: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('hid.isClaimed', deviceInfo);
            },
            ...
            sendData: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('hid.sendData', deviceInfo);
            },
            ...
            readData: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('hid.readData', deviceInfo);
            },
            ...
            sendFeatureReport: function(deviceInfo) {
                return _ep.websocket.dataPromise('hid.sendFeatureReport', deviceInfo);
            },
            ...
            getFeatureReport: function(deviceInfo) {
                return _ep.websocket.dataPromise('hid.getFeatureReport', deviceInfo);
            },
            ...
            openStream: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('hid.openStream', deviceInfo);
            },
            ...
            closeStream: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('hid.closeStream', deviceInfo);
            },
            ...
            releaseDevice: function(deviceInfo) {
                ...
                return _ep.websocket.dataPromise('hid.releaseDevice', deviceInfo);
            }
        },
        ...
        file: {
             ...
             * Due to security reasons, paths are limited to the ep data directory unless overridden via properties file.
             *
             * @param {string} path Relative or absolute directory path. Must reside in ep data directory or a white-listed location.
            ...
            list: function(path, params) {
                var param = _ep.tools.extend({ path: path }, params);
                return _ep.websocket.dataPromise('file.list', param);
            },
             ...
             * Due to security reasons, paths are limited to the ep data directory unless overridden via properties file.
             *
             * @param {string} path Relative or absolute file path. Must reside in ep data directory or a white-listed location.
             * @param {Object} [params] Object containing file access parameters
             *  @param {boolean} [params.sandbox=true] If relative location from root is only available to the certificate's connection, otherwise all connections
             *  @param {boolean} [params.shared=true] If relative location from root is accessible to all users on the system, otherwise just the current user
            ...
            read: function(path, params) {
                var param = _ep.tools.extend({ path: path }, params);
                return _ep.websocket.dataPromise('file.read', param);
            },
            ...
            write: function(path, params) {
                var param = _ep.tools.extend({ path: path }, params);
                return _ep.websocket.dataPromise('file.write', param);
            },
            ...
            remove: function(path, params) {
                var param = _ep.tools.extend({ path: path }, params);
                return _ep.websocket.dataPromise('file.remove', param);
            },
            ...
            startListening: function(path, params) {
                ...
                var param = _ep.tools.extend({ path: path }, params);
                return _ep.websocket.dataPromise('file.startListening', param);
            },
            ...
            stopListening: function(path, params) {
                var param = _ep.tools.extend({ path: path }, params);
                return _ep.websocket.dataPromise('file.stopListening', param);
            },
            ...
            setFileCallbacks: function(calls) {
                _ep.file.fileCallbacks = calls;
            }
        },

        /**
         * Calls related to networking information
         * @namespace ep.networking
         * @since 2.1.0
         */
        networking: {
            /**
             ...
             *
             * @memberof ep.networking
             * @since 2.1.0
             */
            device: function(hostname, port) {
                // Wrap 2.0
                if (_ep.tools.isVersion(2, 0)) {
                    return _ep.compatible.networking(hostname, port, null, null, function(data) {
                        return { ip: data.ipAddress, mac: data.macAddress };
                    });
                }
                // Use 2.1
                return _ep.websocket.dataPromise('networking.device', {
                    hostname: hostname,
                    port: port
                });
            },
            ...
            devices: function(hostname, port) {
                ...
                // Use 2.1
                return _ep.websocket.dataPromise('networking.devices', {
                    hostname: hostname,
                    port: port
                });
            }
        ...
        security: {
             ...
             *
             * @memberof ep.security
             */
            setCertificatePromise: function(promiseHandler) {
                _ep.security.certHandler = promiseHandler;
            },
             ...
             *
             * @example
             *  ep.security.setSignaturePromise(function(dataToSign) {
            ...
            setSignaturePromise: function(promiseFactory) {
                _ep.security.signatureFactory = promiseFactory;
            },

            /**
             * Set which signing algorithm EP will check signatures against.
             *
            ...
            setSignatureAlgorithm: function(algorithm) {
                ...
                if (!_ep.compatible.algorithm()) {
                    return;
                }

                if (["SHA1", "SHA256", "SHA512"].indexOf(algorithm.toUpperCase()) < 0) {
                    _ep.log.error("Signing algorithm '" + algorithm + "' is not supported.");
                } else {
                    _ep.security.signAlgorithm = algorithm;
                }
            },

            /**
             * Get the signing algorithm EP will be checking signatures against.
             *
            ...
            getSignatureAlgorithm: function() {
                return _ep.security.signAlgorithm;
            }
        },
        ...
        api: {
            /**
             * Show or hide EP api debugging statements in the browser console.
             *
             * @param {boolean} show Whether the debugging logs for EP should be shown. Hidden by default.
             * @returns {boolean} Value of debugging flag
             * @memberof ep.api
             */
            showDebug: function(show) {
                return (_ep.DEBUG = show);
            },

            /**
             * Get version of connected Ezee Print application.
             *
             * @returns {Promise<string|Error>} Version number of Ezee Print.
             *
             * @memberof ep.api
             */
            getVersion: function() {
                return _ep.websocket.dataPromise('getVersion');
            },

            /**
             * Checks for the specified version of connected Ezee Print application.
             *
            ...
            /**
             * Change the promise library used by EP API.
            ...
            setPromiseType: function(promiser) {
                _ep.tools.promise = promiser;
            },

            /**
             * Change the SHA-256 hashing function used by EP API.
             * Should be called before any initialization to avoid possible errors.
             *
...
(function() {
    if (typeof define === 'function' && define.amd) {
        ...
        try {
            ...
            ep.api.setSha256Type(function(data) {
