                    ...
                    {foreach item=row from=$val key=t}
                        {if $row->cargo_status_code!='CA' && $row->cargo_status_code!='CR'}
                            <tr>
                                ...
                                <td align="center" nowrap><a href='javascript:;' title="more details" onclick='getCargoDetails("{$row->cargo_code}")'>{$row->cargo_code}</a>{if $namespace == 'ktwoklogistics' || $namespace == 'cargodemo' || $namespace == 'rmtcargo'}<br>PNO: {$row->private_mark_number}{/if}</td>
                ...
                {foreach item=row from=$result key=t}
                    {if $row->cargo_status_code!='CA' && $row->cargo_status_code!='CR'}
                        <tr>
                            ...
                            {if $namespaceCode=='seenutransports'||$namespaceCode=='seenucargo' }
                                <td align="center"nowrap> {$row->transit_alias_code}</td>
                                <td align="left" nowrap>{$row->transit_trip_date|date_format:"d-m-Y"|default:'-'} </td>
                                ...
                                <td align="left" nowrap> {$row->pod_transaction_date|date_format:"d-m-Y"|default:'-'}</td>
                            {/if}
