{if $smarty.request.code != ''}
    <ul class="nav nav-tabs" id="tabs_a">                                            
        <li class="active"><a data-toggle="tab" href="#ogpl-notloaded-tab">New {lang('lr')}</a></li>
        <li><a data-toggle="tab" href="#ogpl-loaded-tab">Loaded {lang('lr')}</a></li>
    </ul>
{/if}
                ...
                <form onsubmit="return false;">
                    <div class="row">
                        <div class="form-group col-md-2">
                            <label for="ogpl-load-from">From Date</label>
                        ...
                        <div class="form-group col-md-2">
                            <label for="ogpl-load-to">To Date</label>
                            <div class="input-group">
                        ...
                        <div class="form-group col-md-2 hide">
                            <label for="ogpl-load-from_org">Branch</label>
                        ...
                        <div class="form-group col-md-2">
                            ...
                            <div class="input-group">
                                ...
                                <button type="button" class="btn btn-success" onclick="ogplLRSearchReport();">Generate</button>&nbsp;
...
<script>
    ...
    $('#ogpl-load-from').datepicker("setDate", '-2m');
    ...
    function selectAllPreBooking(cb) {
        var $cb = $(cb);
    ...
    function loadOGPL() {
        var ogplRoutes = {};
        var ogplPrebookRoutes = {};
        var loadToHub = $('#ogpl-load-hub').is(':checked');
        
        $('.inp_error').removeClass('inp_error');
        
        if($('#load-ogpl-list .ogpl-new-items:checked, #prebook-ogpl-list .prebook-ogpl-items:checked').length === 0) {
        ...
        var load_type = [];
        var transitStation = '';
        var transitOrganization = '';
        $('#load-ogpl-list .ogpl-new-items:checked').each(function (i, cb) {
            var $cb = $(cb);
            if ($cb.data('to')) {

                var transitorg = $('#load-branch-sel').val();
                {*if (transitorg == '') {
                ...
                load_type.push($cb.data('loadtype'));
                transitStation = $cb.data('transitstation');
                transitOrganization = $cb.data('transitbranch');
        ...
        $('#prebook-ogpl-list .prebook-ogpl-items:checked').each(function (i, cb) {
            var $cb = $(cb);
            if ($cb.data('to')) {
                ...
                var route = $cb.data('from') + '-' + $cb.data('to') + '-' + transitorg;
                if (!ogplPrebookRoutes[route]) {
                    ogplPrebookRoutes[route] = {};
                    ogplPrebookRoutes[route].cargoList = [];
                    ogplPrebookRoutes[route].from = $cb.data('from');
                    ogplPrebookRoutes[route].to = $cb.data('to');
                    ogplPrebookRoutes[route].transitOrganization = transitorg;
                }
                ogplPrebookRoutes[route].cargoList.push($cb.val());
            } else {
                $('#load-ogpl-action-state').removeClass('alert-success').addClass('alert-danger');
                $('#load-ogpl-action-state').html($cb.val() + ' not valid for the selected Branch. Kinldy check from and to stations');
                $('#load-ogpl-action-state').show();
        ...
        if ($.isEmptyObject(ogplRoutes) && $.isEmptyObject(ogplPrebookRoutes)) {
            return false;
        }
        ...
        data.prebookLoadList = _.values(ogplPrebookRoutes);

        data.additionalAttribute = {};
        if ($.inArray('FTL', load_type) > -1 || $.inArray('PTL', load_type) > -1) {
            data.additionalAttribute.FTL_PTL_TRANSIT_FLAG = 1;
            
            if (OGPLFormData.cargoActivityType.code == 'OTFD') {
                data.fromStation = {};
                data.fromStation.code = transitStation;

                data.toStation = {};
                data.toStation.code = transitStation;

                data.fromOrganization = {};
                data.fromOrganization.code = transitOrganization;

                data.toOrganization = {};
                data.toOrganization.code = transitOrganization;
            }
        }
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: base_url + 'cargo/load-out-for-delivery-list',
            data: data,
    ...
    function getLoadedOGPL() {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: base_url + 'cargo/loaded-out-for-delivery-list',
            data: data,
            success: function (response) {
    ...
    function ogplLRSearchReport() {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: base_url + 'cargo/booked-out-for-delivery-search-list',
            data: data,
            success: function (response) {
