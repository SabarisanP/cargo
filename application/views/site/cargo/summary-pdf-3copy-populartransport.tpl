            ...
            <tbody>
                <tr>
                    <td rowspan="2" width="150">
                        <img src="assets/img/clientlogo/populartransport.png" alt="logo" height="30">
                    </td>
        ...
        <table width="100%" cellpadding="2" class="bordered">
            <tbody>
                ...
                <tr>
                    ...
                    <td align="right" style="height: 16px; line-height: 16px;">{$invoiceDetail[0]|inr_format}</td>
                    <td align="right" colspan="2">(Loading Charges:{if $summary->vendorUser->name!=''} - {else} {$summary->totalHandlingAmount|inr_format} {/if} Unloading Charges:{if $summary->vendorUser->name!=''} - {else} {$summary->totalUnloadHandlingAmount|inr_format} {/if}) <br/></td>
                    <td align="right" style="height: 16px; line-height: 16px;">
                        {if $summary->vendorUser->name!=''} 
                            - 
                        {else}
                            {($summary->totalHandlingAmount+$summary->totalUnloadHandlingAmount+$summary->pickupHandlingAmount+$summary->deliveryHandlingAmount+$summary->serviceCharge+$summary->serviceTax+$summary->hamaliCharge+$summary->passCharge+$summary->docketCharge + $addons_mapped['PODCH']->value)|inr_format}
                        {/if}
                    </td>
                </tr>
                <tr>
                    ...
                    <td align="right">
                        {if $summary->vendorUser->name!=''} 
                            - 
                        {else}
                            {($summary->totalAmount+$summary->totalHandlingAmount+$summary->totalUnloadHandlingAmount+$summary->pickupHandlingAmount+$summary->deliveryHandlingAmount+$summary->serviceCharge+$summary->serviceTax+$summary->hamaliCharge+$summary->passCharge+$summary->docketCharge + $addons_mapped['PODCH']->value)|inr_format}
                        {/if}    
                    </td>
                </tr>
