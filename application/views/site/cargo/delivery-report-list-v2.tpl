<table class="" width="95%">
    <tr>
        <td colspan="2" align="center"><span class="bold">Cargo Delivery Report</span></td>
    </tr>
    <tr>
        <td width="50%">
        ...
        <table class="table table-bordered noprint">
            <thead>
                {foreach item=row from=$summary key=paytype}
                    {if $row['code'] =="TPPAD"}
                        <tr class="success">
                            <td align="center" class="bold">Topay Then Paid</td>
                            <td align="center" class="bold">Amount</td>
                        </tr>
                    </thead>
                    {* // *}
...
<div class="row">
    ...
    <div class="col-md-6">
        <table class="table table-bordered noprint">
            ...
            {foreach item=row from=$summary key=paytype}
                <tr>
                    <td class="bold">{$cargo_payment_status[$paytype]}{if $paytype =="PAD"}(a)
                    {elseif $paytype =="TOP"} (b)
                        {elseif $paytype =="INP"} (c) 
                        {/if}</td>
    ...
    <table id="dt_adv_booking_report_table" class="table table-bordered " style="width:99%">
        ...
        <tbody class="body_table">
                        ...
                        {if $row->cargo_status_code!='CA' && $row->cargo_status_code!='CR'}
                            <tr>
                                <td align="left" nowrap>
                                    {capture assign=timestamp}{$row->booked_at|strtotime}{/capture}{$timestamp|date_format:"%d %b %Y %I:%M %p"}
                                </td>
                                <td align="center" nowrap><a href='javascript:;' title="more details"
                                        onclick='getCargoDetails("{$row->cargo_code}")'>{$row->cargo_code}</a></td>
                                <td align="center">
                                    {if $namespaceCode == "dishashree"}{$row->total_item_count}/{$row->total_unit_value}{else}{$row->total_item_count}{/if}
                                </td>
            ...
            <tr class="active">
                <td align="center"></td>
                <td align="left"></td>
                <td align="center">{if $namespaceCode == "dishashree"}{$tot_items}/{$tot_unit}{else}{$tot_items}{/if}
                </td>
                ...
                <td align='right' class="bold" nowrap>{$t_other_tot_amount|inr_format} <i
                        class="fa fa-info-circle invisible"></i></td>
...
<div class="clear_fix_both text-right">
    <div class="btn-group btn-group-sm">
        <button class="btn btn-default" onclick="exportCargoDeliveryExcel_v2()"><i
