...
{if $SinglrList->additionalAttribute->TRANSACTION_RECEIPT == $result->code && $SinglrList->transactionType->code == "MCHC"} 
 ...
 {/if} 
{/foreach}
{assign var=bookingFlag value="false"}
{assign var=AmtValue value= 0}
{assign var=DeliveryFlag value="false"}


{foreach $ReceiptLr[0]->cargoAddonsDetails as $SinglrList}

{if $SinglrList->addonsType->code =="MCNHCHBL"}
    {assign var=bookingFlag value="true"}
    {assign var=AmtValue value= $AmtValue + $SinglrList->value}
{/if}

{if $SinglrList->addonsType->code =="MCNHCHDL"}
    {assign var=DeliveryFlag value="true"}
    {assign var=machineHandle value= 0}
    {assign var =machineHandle  value= $machineHandle + $SinglrList->value}
{/if}
{/foreach}
...
<table class="bordered" cellpadding="2" id="tab-2" width="100%">
 ...
 {if ($bookingFlag == "true") &&  ($row->paymentType->code == "TPPAD") }
            <td align="right" width="12%">{($row->deliveryHandlingAmount+$row->pickupHandlingAmount+$row->hamaliCharge+$row->passCharge+$row->docketCharge+$row->totalAmount+$row->totalHandlingAmount+$row->totalUnloadHandlingAmount) + ($AmtValue)|amountToRupeesPaise:'RUPEES'}</td>
            <td align="right" width="12%">{($row->deliveryHandlingAmount+$row->pickupHandlingAmount+$row->hamaliCharge+$row->passCharge+$row->docketCharge+$row->totalAmount+$row->totalHandlingAmount+$row->totalUnloadHandlingAmount) + ($AmtValue)|amountToRupeesPaise:'RUPEES'}</td> 
        {else}
...
{assign var = tempMachine value= 0 }
{if ($result->cargoBookings[0]->paymentType->code== "TPPAD" ) && ($DeliveryFlag == "true")}
{assign var = tempMachine value= $tempMachine + $machineHandle }
{else}
    {assign var = tempMachine value= $tempMachine + $result->machineHandlingCharge }
{/if}
{assign var=net_total value=$net_total+($total_lr_amt+$tempMachine +$result->demurrageCharge+$rounf_off_val)}
        ...
        <td colspan="5">
            <table style="padding:5px 4px;">
                ...
                <tr>
                    ...
                    {else} *}
                        {* <td class="dotted-border-bottom" colspan="3">{$tempMachine|inr_format}</td> *}
                        <td class="dotted-border-bottom" colspan="1" align = "center">{$tempMachine|inr_format}</td>
                    <td class="dotted-border-bottom" colspan="2" align = "left"><b>{if ($tempMachine>0 || $transactionAmount>0) }(Machine Handling Charge){/if}</b></td>
                    {* {/if} *}
                </tr>
                <tr>
        ...
        <td colspan="2">
            ...
            <table style="padding:5px 5px;" class="bordered">
                ...
                {* {if ($result->cargoBookings[0]->paymentType->code== "PAD" && $transactionAmount != 0)}
                    ...
                    {else} *}
                    <td align="right">{$tempMachine|amountToRupeesPaise:'RUPEES'}</td>
                    <td align="right">{$tempMachine|amountToRupeesPaise:'PAISE'}</td>
                    {* {/if} *}
                </tr>
                <tr>
                ...
                {if ($result->cargoBookings[0]->paymentType->code== "PAD"  &&  $extraCharges)}
                    ...
                    {else}
                    <td align="right" class="bold" style="font-size: 9px;">{($total_lr_amt+$tempMachine +$result->demurrageCharge+$rounf_off_val-$result->tdsAmount)|amountToRupeesPaise:'RUPEES'}</td>
                    <td align="right" class="bold" style="font-size: 9px;">{($total_lr_amt+$tempMachine +$result->demurrageCharge+$rounf_off_val-$result->tdsAmount)|amountToRupeesPaise:'PAISE'}</td>
                {/if}
                    </tr>
            ...
            <td width="70%">
                <table style="padding:5px 4px;">
                    ...
                    {if ($result->cargoBookings[0]->paymentType->code== "PAD" ) &&  $extraCharges}
                        ...
                        {else}
                        <td class="" colspan="5">Received Amount (Rs) :&nbsp;<b>{($total_lr_amt+$tempMachine +$result->demurrageCharge+$rounf_off_val -$result->tdsAmount)|amountToRupeesPaise:'RUPEES'} / {($total_lr_amt+$tempMachine +$result->demurrageCharge-$result->tdsAmount)|amount_in_words|ucwords} Only </b></td>
                    {/if}
