        ...
        <tbody>
            {foreach item=row from=$result key=k}
                <tr>
                    ...
                    <td>
                        <span class="badge badge-success">{$row->paymentStatus->name}</span>
                    ...
                    <td align="center">
                        {if $action_rights['CONTRA-ALLOW-ALL-BRANCH'] == 1 || (isNotNull($row->vendorContactOrganization->code) && $row->vendorContactOrganization->code == $loggedin_user_org)}
                            <button class="btn-app btn-success" type="button" onclick="updateContraStatus('{$row->code}', 'ACK');">Acknowledge</button>
                        {/if}
                    </td>
                </tr>
            {foreachelse}
...
<script>
    var contraAckData = {$result|json_encode}
